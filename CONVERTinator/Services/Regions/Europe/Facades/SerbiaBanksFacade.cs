...
{
    internal class SerbiaBanksFacade : IExchangeRateProvider
    {
        private readonly List<IExchangeRateProvider> _localBanks;

        public SerbiaBanksFacade()
        {
            _localBanks = new List<IExchangeRateProvider>
            {
                // Universal provider configured for Serbian Dinar (RSD)
                new RegionalFloatRatesProvider("rsd", "National Bank of Serbia (via FR)")
            };
        }
        ...
        public async Task<List<Currency>> GetRatesAsync()
        {
            var tasks = _localBanks.Select(async bank =>
            {
                ...
                {
                    Console.WriteLine($"[SERBIA FACADE WARN] Provider failed: {ex.Message}");
            ...
            decimal rsdToUsdCrossRate = usdRateObj.Value;
            ...
            foreach (var rate in validRates)
            {
                ...
                var cleanRate = RateNormalizer.NormalizeForeignBase(rate, rsdToUsdCrossRate);
            ...
            allNormalizedRates.Add(new Currency
            {
                Code = "RSD",
                Name = "Serbian Dinar",
                Value = 1m / rsdToUsdCrossRate,
                Source = "Serbia Facade"
            });
